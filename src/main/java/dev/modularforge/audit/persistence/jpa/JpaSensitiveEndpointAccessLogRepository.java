...
public interface JpaSensitiveEndpointAccessLogRepository extends SensitiveEndpointAccessLogRepository, JpaRepository<SensitiveEndpointAccessLog, Long> {
    @Override <S extends SensitiveEndpointAccessLog> S saveAndFlush(S entity);
    @Override void flush();
    ...
    @Query("SELECT CAST(s.createdAt AS LocalDate) as date, COUNT(s) as count FROM SensitiveEndpointAccessLog s " +
           "WHERE s.createdAt >= :since GROUP BY CAST(s.createdAt AS LocalDate) ORDER BY date DESC")
